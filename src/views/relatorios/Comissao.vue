 ...
 <div v-if="store.dataErrada" @click="store.dataErrada=false"
    ...
    <Progress v-if="store.Progress"/> 
      ...
      <div v-if="store.Progress == false" style="margin-top: -30px;">
        ...
        <div  v-if="store.relMondado?.length > 0"  class="table-light table-responsive" style="  background-color: #ffffff;" v-bind:style="[!store.detectar_mobile() ? {'width': '630px' } : '630px']">
            <table role="table" aria-busy="false" aria-colcount="5" class="table b-table table-hover" id="__BVID__310">
                <thead role="rowgroup" class="">
                    <tr role="row" class="">
                        <th role="columnheader" scope="col" aria-colindex="1" class=""><div>Funcionario</div></th>
                        <th role="columnheader" scope="col" aria-colindex="2"  style="text-align: end;"><div>Qtde</div></th>                       
                        <th role="columnheader" scope="col" aria-colindex="3"  style="text-align: end;"><div>Total Vendas</div></th>                       
                        <th role="columnheader" scope="col" aria-colindex="4"  style="text-align: end;"><div>Desconto</div></th>                       
                        <th role="columnheader" scope="col" aria-colindex="5"  style="text-align: end;"><div>% desc</div></th>                       
                        <th role="columnheader" scope="col" aria-colindex="6"  style="text-align: end;"><div>Valor Final</div></th>                       
                        <th role="columnheader" scope="col" aria-colindex="7"  style="text-align: end;"><div>Comissao</div></th>                       
                    </tr>
                </thead>
                <tbody v-for="(item, i) in store.relMondado[store.indexVendas].REGISTRO" :key="item.name" role="rowgroup">
                    <tr role="row" class="">
                        <td aria-colindex="1" role="cell" class="" style=" font-weight: bold ; ;" >{{ item.loginfuncionario }}</td>
                        <td aria-colindex="1" role="cell" class="" style=" font-weight: bold ; text-align: center;" >-</td>
                        <td aria-colindex="1" role="cell" class="" style=" font-weight: bold ; text-align: end;" >{{ formataDinheiro(item.valorfinal) }}</td>
                        <td aria-colindex="1" role="cell" class="" style=" font-weight: bold ; text-align: center;" >{{ formataDinheiro(item.vl_desconto) }}</td>
                        <td aria-colindex="1" role="cell" class="" style=" font-weight: bold ; text-align: end;" >{{ item.percentualdesconto }}%</td> 
                        <td aria-colindex="1" role="cell" class="" style=" font-weight: bold ; text-align: end;" >{{ formataDinheiro(item.valorfinal) }}</td>
                        <td aria-colindex="2" role="cell" style="text-align: end; font-weight: bold ;"  > <span :style="[item.COR == 'red' ? {'color': '#B22222' } : '#000000' ]" 
                                                                                                                ...
                                                                                                                >R$ {{ formataDinheiro(item.valorcomissaofuncionario) }}</span> </td> 
                    </tr>
                </tbody>
            </table>
        </div>
  ...
  store.filtro= { 
    ...
    if(storeLogin.empresas?.sucess && store.filtro.dataFinal >= store.filtro.dataInicial){
        ...
        store.relContas = []
        store.Progress = true        
        getTypeRel(storeLogin.empresas.databasecliente,store.filtro.empresa,store.filtro.dataInicial,store.filtro.dataFinal,6)
    }else{
